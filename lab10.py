...
def letterCounts(fname):
    ...
    for char in data:
        if char.isalpha():  # Check if the character is a letter
            char = char.lower()  # Convert to lowercase to count
    ...
    # Count non-letters
    non_letter_count = len([char for char in data if not char.isalpha()])

    # Sort the dictionary
    sorted_letters = sorted(letters.items(), key=lambda x: (x[1], x[0] != 'non-letter'), reverse=True)

    # Print the list of pairs
    result = [(count, char) for char, count in sorted_letters if count > 0] + [('non-letter', non_letter_count)]
    print(result)
